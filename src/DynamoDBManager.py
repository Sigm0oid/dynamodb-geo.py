...
class DynamoDBManager:
    ...
    def put_Point(self, putPointInput: 'PutPointInput'):
        ...
        response = ""
        # prepare the non key attributes first
        item = self.dict_to_item(putPointInput.ExtraFields)

        item[self.config.hashKeyAttributeName] = {"N": str(hashKey)}
        item[self.config.rangeKeyAttributeName] = {
            "S": putPointInput.RangeKeyValue}
        item[self.config.geohashAttributeName] = {'N': str(geohash)}
        item[self.config.geoJsonAttributeName] = {"S": "{},{}".format(
