...
Author(s): Hamza Rhibi, Brandon Sneider
"""
...
class GeoTableUtil:
    ...
    def create_table(self, CreateTableInput: "Dict with parameters to create the table"):
        ...
        try:
            response = self.config.dynamodbResource.describe_table(
                TableName=self.config.tableName)
            # table exists...bail
            print("Table [{}] already exists. Skipping table creation.".format(
        ...
        table = self.config.dynamodbResource.create_table(**CreateTableInput)

        print("Waiting for table [{}] to be created".format(
            self.config.tableName))
